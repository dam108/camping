...
public class MainGrafico extends javax.swing.JFrame {
    ...
    public void crearBotones() {
        ...
        for (int i = 0; i < Camping.parcelas.size(); i++) {
            ...
            tablero[i].setFont(new java.awt.Font("Tahoma", 0, 25)
            ...
            if (Camping.parcelas.get(i).ocupado) {
                ...
                tablero[i].setBackground(Color.DARK_GRAY);
            }
            ...
            if (numBoton >= 0 && numBoton < 10) {
                tablero[i].setIcon(new javax.swing.ImageIcon(
                        "img\\Tienda.PNG"));
    ...
    public void Bungalow() {
        ...
        if (Camping.parcelas.get(numBoton) instanceof Bungalow) {
            ...
            if (!Camping.parcelas.get(numBoton).ocupado) {
                try {
                    do {
                        dni = JOptionPane.showInputDialog(this,
                                "Introduce el DNI del huesped");
                    } while (!comprobarDNI(dni));
                    try {
                        do {
                            ad = JOptionPane.showInputDialog(this,
                                    "Introduce el numero de adultos");
                ...
                precio = ((Bungalow) Camping.parcelas.get(numBoton))
                        .checkOut(p);
                System.out.println(precio);
                JOptionPane.showMessageDialog(this, "El importe es de " 
                        + precio);
                tablero[numBoton].setBackground(Color.DARK_GRAY);
            }
        }
    }

    public String Caravana(String dni) {
        double precio;
        // Caravana
        if (Camping.parcelas.get(numBoton) instanceof Caravana) {
            ...
            if (!Camping.parcelas.get(numBoton).ocupado) {
                ...
                precio = ((Caravana) Camping.parcelas.get(numBoton))
                        .checkOut(p);
                if (precio == -1) {
                    JOptionPane.showMessageDialog(this,
                    "No se puede hacer checkOut antes de"+
                    p.getDiasMinimosCaravana()+" dias de estancia");
                } else {
                    tablero[numBoton].setBackground(Color.DARK_GRAY);
                    JOptionPane.showMessageDialog(this, "El importe es de "
                            + precio);
    ...
    public String Tienda(String dni) {
    ...
    private void JB_Edit_ParamActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_JB_Edit_ParamActionPerformed
        try{
            int abrirF = JOptionPane.showConfirmDialog(this, "¿Quieres"
                    + " modificar el fichero de configuración?."
                    + " Si lo haces tienes que darle al boton de actualizar"
                    + " despues de modificar y guardar el archivo");
            if (abrirF == 0){
    ...
    private void JB_AyudaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_JB_AyudaActionPerformed
        JOptionPane.showMessageDialog(this, "Funcionamiento:\n"
                + "Este programa permite la gestión de las entradas y salidas"
                + " del camping.\n"
                ...
                + "Editar Parametros: Se nos abre un archivo con la "
                + "configuracion para que podemos modificarla en caso"
                + " de necesitarlo.\n"
                + "Actualizar Parametros: Sirve para que el programa recargue"
                + " la configuracion del archivo anterior justo despues de la"
                + " modificacion.\n"
    ...
    public static boolean comprobarDNI(String dni){
        if(dni.length() == 9){
            ...
            if (!Character.isLetter(letra.charAt(0))) return false;
